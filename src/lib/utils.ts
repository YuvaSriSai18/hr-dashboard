...
import { departments } from '@/types'; // Added import
...
export function generateMockProjects(count: number = 2): Project[] {
  const statuses: Project["status"][] = ["Completed", "In Progress", "On Hold", "Cancelled"];
...
export function transformDummyUserToUser(dummyUser: any): User {
   const department = departments.includes(dummyUser.company?.department as Department) ? dummyUser.company?.department as Department : getRandomDepartment();
  return {
    id: dummyUser.id,
    ...
    bio: dummyUser.company?.department && dummyUser.company?.title ? `An experienced ${dummyUser.company.title} in the ${dummyUser.company.department} department. Dedicated and results-oriented professional.` : `A valuable member of the team, bringing enthusiasm and a unique skill set.`,
